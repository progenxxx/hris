...
const CancelRestDayDetailModal = ({ cancelRestDay, onClose, onStatusUpdate, userRoles = {} }) => {
    ...
    const handleStatusChange = (status) => {
        ...
        if (typeof onStatusUpdate === 'function') {
            onStatusUpdate(cancelRestDay.id, data);
        }
        setProcessing(false);
    };
                        ...
                        <div className="sm:flex sm:items-start">
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">
                                    Cancel Rest Day Details #{cancelRestDay.id}
                                </h3>
                                ...
                                <div className="mt-4 bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-2 sm:gap-4 sm:px-6 rounded-md">
                                    ...
                                    <div className="mt-1 text-sm text-gray-900 sm:mt-0">
                                        {cancelRestDay.employee ? 
                                            `${cancelRestDay.employee.Lname}, ${cancelRestDay.employee.Fname} ${cancelRestDay.employee.MName || ''}` 
                                            : 'N/A'}
                                    </div>
                                    
                                    <div className="text-sm font-medium text-gray-500">Department</div>
                                    <div className="mt-1 text-sm text-gray-900 sm:mt-0">
                                        {cancelRestDay.employee?.Department || 'N/A'}
                                    </div>
                                    
                                    <div className="text-sm font-medium text-gray-500">Job Title</div>
                                    <div className="mt-1 text-sm text-gray-900 sm:mt-0">
                                        {cancelRestDay.employee?.Jobtitle || 'N/A'}
                                    </div>
                                </div>
                                
                                {/* Cancel rest day details section */}
                                <div className="mt-4 bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-2 sm:gap-4 sm:px-6 rounded-md">
                                    <div className="text-sm font-medium text-gray-500">Rest Day to Cancel</div>
                                    <div className="mt-1 text-sm text-gray-900 sm:mt-0">
                                        {cancelRestDay.rest_day_date ? formatDate(cancelRestDay.rest_day_date) : 'N/A'}
                                    </div>
                                    
                                    <div className="text-sm font-medium text-gray-500">Replacement Work Date</div>
                                    <div className="mt-1 text-sm text-gray-900 sm:mt-0">
                                    ...
                                    <div className="mt-1 text-sm text-gray-900 sm:mt-0">
                                        {cancelRestDay.created_at ? 
                                            formatDateTime(cancelRestDay.created_at) 
                                            : 'N/A'}
                                    </div>
                                </div>
                                ...
                                <div className="mt-4">
                                    ...
                                    <div className="border rounded-md p-3 bg-gray-50 text-sm text-gray-900">
                                        {cancelRestDay.reason || 'No reason provided'}
                                    </div>
                                </div>
                                
                                {/* Approval Status Section */}
                                {cancelRestDay.approved_at && (
                                            ...
                                            <div className="flex items-start justify-between">
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <div className="text-xs text-gray-500 mt-1">
                                                        {formatDateTime(cancelRestDay.approved_at)}
                                                    </div>
                                                </div>
                                            </div>
                                            
                                            {cancelRestDay.remarks && (
                                                <div className="border border-gray-200 rounded p-2 text-sm text-gray-700 bg-white">
                                                    <span className="font-medium">Remarks:</span> {cancelRestDay.remarks}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                ...
                                {canApprove && cancelRestDay.status === 'pending' && (
                                ...
                                {userRoles.isSuperAdmin && cancelRestDay.status === 'pending' && (
                                    <div className="mt-6 border-t border-gray-200 pt-4">
...
export default CancelRestDayDetailModal;