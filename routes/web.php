...
Route::middleware(['auth', 'verified', 'role:hrd_manager,superadmin'])->group(function () {
    ...
    Route::post('/employee-schedules/import', [EmployeeScheduleController::class, 'importImproved'])
    ...
    Route::get('/employee-schedules/debug', function () {
        try {
            // Check if table exists
            $tableExists = Schema::hasTable('employee_schedules');
            
            // Get table structure
            $columns = [];
            if ($tableExists) {
                $columns = Schema::getColumnListing('employee_schedules');
            }
            ...
            if ($tableExists) {
                $schedules = EmployeeSchedule::with(['employee:id,idno,Fname,Lname,Department'])
                    ->limit(5)
                    ->get()
                    ->toArray();
                $schedulesCount = EmployeeSchedule::count();
            }
            ...
            if ($employeesTableExists) {
                $employees = Employee::select('id', 'idno', 'Fname', 'Lname', 'Department')
                    ...
                    ->get()
                    ->toArray();
                $employeesCount = Employee::where('JobStatus', 'Active')->count();
            }
            
            return response()->json([
                'debug_info' => [
                    'employee_schedules_table_exists' => $tableExists,
                    'employee_schedules_columns' => $columns,
    ...
    })->name('employee-schedules.debug');

    /*
    |--------------------------------------------------------------------------
    | Payroll Schedule Integration Routes
    |--------------------------------------------------------------------------
    */
    
    // Main Payroll Schedule Integration Page
    Route::get('/payroll-schedule-integration', [PayrollScheduleController::class, 'index'])
        ->name('payroll-schedule-integration.index');
    
    // Sync payroll data with schedules and attendance
    Route::post('/payroll-schedule-integration/sync', [PayrollScheduleController::class, 'syncPayrollData'])
        ->name('payroll-schedule-integration.sync');
    
    // Get detailed sync information for a specific employee
    Route::get('/payroll-schedule-integration/employee/{employeeId}/details', [PayrollScheduleController::class, 'getEmployeeSyncDetails'])
        ->name('payroll-schedule-integration.employee-details')
        ->where('employeeId', '[0-9]+');
    
    // Generate payroll summaries for employees without existing records
    Route::post('/payroll-schedule-integration/generate', [PayrollScheduleController::class, 'generatePayrollSummaries'])
        ->name('payroll-schedule-integration.generate');
    
    // Export payroll schedule integration data
    Route::post('/payroll-schedule-integration/export', [PayrollScheduleController::class, 'export'])
        ->name('payroll-schedule-integration.export');
    ...
    // Get benefits details for payroll summary
    Route::get('/api/payroll-summaries/{id}/benefits-details', [PayrollSummariesController::class, 'getBenefitsDetails'])
        ->name('payroll-summaries.benefits-details')
        ->where('id', '[0-9]+');

    // Get deductions details for payroll summary
    Route::get('/api/payroll-summaries/{id}/deductions-details', [PayrollSummariesController::class, 'getDeductionsDetails'])
        ->name('payroll-summaries.deductions-details')
        ->where('id', '[0-9]+');

    // Get comprehensive financial summary
    Route::get('/api/payroll-summaries/{id}/financial-summary', [PayrollSummariesController::class, 'getFinancialSummary'])
        ->name('payroll-summaries.financial-summary')
        ->where('id', '[0-9]+');
